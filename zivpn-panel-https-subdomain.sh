...
# 💡 HTTPS MODIFICATION: NGINX + CERTBOT ADDED for sell.zivpn-panel.cc (Now configurable)
set -euo pipefail
...
say(){ 
    ...
    echo -e "${C}🚨 Web Panel ကို Nginx/Certbot ဖြင့် HTTPS (Domain ပြောင်းလဲသတ်မှတ်နိုင်သည်) သို့ ပြောင်းလဲနေပါသည်။${Z}"
    echo -e "$LINE"
...
# ===== PROMPT FOR DOMAIN (NEW BLOCK) =====
echo -e "${G}🌐 Web Panel အတွက် HTTPS အသုံးပြုမည့် Domain/Subdomain ကို ထည့်သွင်းပါ${Z}"
read -r -p "Web Panel Domain (ဥပမာ: panel.yourdomain.com): " CUSTOM_DOMAIN

if [ -z "${CUSTOM_DOMAIN:-}" ]; then
    echo -e "${R}❌ Domain အမည် မပါဝင်ပါ၊ Script ကို ရပ်ပါမည်။${Z}"
    exit 1
fi
DOMAIN="$CUSTOM_DOMAIN"
echo -e "${G}✅ သင်ရွေးချယ်သော Domain: ${C}$DOMAIN${Z}"
# ===== END PROMPT FOR DOMAIN =====
...
# DOMAIN variable is now set by user input
...
# --- ZIVPN Web Admin Login, VPN Passwords, config.json Update, systemd: ZIVPN (UNCHANGED logic) ---
