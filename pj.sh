...
# Important: Keep existing connections alive during reload
KillMode=mixed
TimeoutStopSec=10
...
function copyText(t,btn){function ok(){if(btn){btn.innerText='✓';btn.disabled=true;setTimeout(()=>{btn.innerText='📋';btn.disabled=false;},800)}};if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(t).then(ok);}else{const ta=document.createElement('textarea');ta.value=t;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);ok();}}
...
<!-- Floating buttons (Telegram / Logout) -->
<div class="fab">
...
<main class="max-w-6xl mx-auto px-4 py-4 space-y-4">
  ...
  <section class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200">
    <div class="grid sm:grid-cols-2 gap-2 text-sm">
      ...
      <div>Server Time: <span id="server-time" class="font-semibold text-green-600">--:--:--</span></div>
    </div>
  </section>

  <!-- Flash messages (Create Account Successfully ✅) -->
...
{{ msg }}
  ...
  <section class="flex gap-2">
    <form method="post" action="/apply">
      <button class="bg-slate-800 hover:bg-slate-700 text-white rounded-xl px-3 py-2 text-sm">⚙️ Apply Config (Reload only)</button>
    </form>
  </section>
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight">
        <thead>
          <tr class="text-slate-600 text-[12px] border-b">
        ...
        <tbody class="tiny">
          ...
          <tr class="border-b hover:bg-slate-50">
            <td class="py-2 px-3 align-top">
              <span class="font-medium text-slate-900">{{r['username']}}</span>
            </td>
            <td class="py-2 px-3 align-top">
              <div class="flex items-start gap-1.5">
                <code class="code-chip px-2 py-1 bg-slate-100 rounded text-xs flex-1 min-w-0 truncate" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded text-xs transition-colors" title="Copy Password">📋</button>
              </div>
              {% if r['days_left'] is not none %}
                <div class="mt-1">
                  {% if r['days_left'] >= 0 %}
                    <span class="badge bg-emerald-100 text-emerald-700 text-xs">{{r['days_left']}} days left</span>
                  {% else %}
                    <span class="badge bg-rose-100 text-rose-700 text-xs">Expired {{-r['days_left']}} days ago</span>
                  {% endif %}
                </div>
              {% endif %}
            </td>
            <td class="py-2 px-3 align-top text-slate-600 text-sm">{{r['expires']}}</td>
            <td class="py-2 px-3 align-top">
              {% if not r['expired'] %}
                <span class="inline-flex items-center gap-1.5 text-emerald-700 text-sm">
            ...
            <td class="py-2 px-3 align-top">
              <div class="flex items-center gap-1.5">
                <button type="button" onclick="fillForm('{{r['username']}}','{{r['password']}}','{{r['expires']}}')" class="px-2 py-1 bg-amber-500 hover:bg-amber-400 text-white rounded text-xs transition-colors">Edit</button>
                <form method=post action="/del/{{r['id']}}" onsubmit="return confirm('Delete {{r['username']}}?')" class="inline">
                  <button class="px-2 py-1 bg-rose-600 hover:bg-rose-500 text-white rounded text-xs transition-colors">🗑️</button>
                </form>
              </div>
            </td>
          </tr>
...
# Apply = RELOAD ONLY (never restart) - with connection preservation
@app.route("/apply", methods=["POST"])
@login_required
def apply():
    try:
        # Use HUP signal only - this reloads config without killing the process
        # This preserves existing UDP connections
        result = subprocess.call([
            "pkill", "-HUP", "-f", "zivpn server"
        ], stdout=DEVNULL, stderr=DEVNULL)
        
        if result == 0:
            flash("✅ Config reloaded successfully - existing connections preserved", "ok")
        else:
            # Fallback to systemctl reload if pkill fails
            subprocess.call(["systemctl","reload",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
            flash("✅ Config applied with reload (no restart)", "ok")
            
    except Exception as e:
        flash(f"❌ Reload attempt failed: {str(e)}", "err")
    return redirect("/")
