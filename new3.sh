...
# 💡 HTTPS MODIFICATION: Added logic to run Flask with self-signed SSL certificate from web.py.
set -euo pipefail
...
# 💡 MODIFIED: stop zivpn-web.service
...
# 💡 UNCHANGED: SSL စိတျဖိုင်တွေ ဖန်တီးနေပါတယ်
...
# 💡 NEW ADDITION: zivpn-web.service (Modified to run web.py directly for SSL context)
...
User=root
WorkingDirectory=/etc/zivpn
# 💡 ExecStart: Python3 ကို တိုက်ရိုက် run ပြီး web.py ထဲမှာ SSL context ကို ကိုင်တွယ်ထားသည်။
...
# 💡 Web Panel (Flask - web.py) (MODIFIED: Added SSL context to app.run)
...
# 💡 HTTPS MODIFICATION: Added logic to run with SSL Context
if __name__ == "__main__":
  CERT_PATH = "/etc/zivpn/zivpn.crt"
  KEY_PATH = "/etc/zivpn/zivpn.key"
  
  # SSL certificate များကို စစ်ဆေးပြီး Flask ကို HTTPS ဖြင့် Run ပါ
  if os.path.exists(CERT_PATH) and os.path.exists(KEY_PATH):
    # Flask ကို SSL context ဖြင့် run ပါ
    # Port 8080 ကို HTTP/HTTPS နှစ်မျိုးလုံး ဖွင့်ထားရန် မျှော်လင့်သည်။ (Production တွင် 443 ကို သုံးပါက၊ 8080 ကို 443 သို့ ပြောင်းရန်လိုသည်)
    app.run(host="0.0.0.0", port=8080, ssl_context=(CERT_PATH, KEY_PATH))
  else:
    # SSL ဖိုင်များ မရှိပါက HTTP ဖြင့် run ပါ (ပုံမှန်အတိုင်း)
...
ufw allow 8080/tcp >/dev/null 2>&1 || true # 💡 UNCHANGED: Web Port
...
# 💡 MODIFIED: zivpn-web.service ကို enable လုပ်ခြင်း
systemctl enable --now zivpn-web.service

IP=$(hostname -I | awk '{print $1}')
echo -e "\n$LINE\n${G}✅ Done${Z}"
echo -e "${C}Web Panel (HTTPS) :${Z} ${Y}https://$IP:8080${Z} (⚠️ Self-Signed Certificate ဖြစ်သောကြောင့် Browser တွင် Warning ပြပါမည်။)"
echo -e "${C}Web Panel (User List) :${Z} ${Y}https://$IP:8080/users${Z}"
